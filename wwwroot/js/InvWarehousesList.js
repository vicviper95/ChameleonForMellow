  ...
  that.find('button[id="updateWarehousesBtn"]').click(function () {
    ...
    $.ajax({
      ...
      success: function () {
        ...
        document.getElementById('getFastMovSkusBttn').disabled = false;
        //document.getElementById('fstMvSendNotifBttn').disabled = false;
...
//getFastMovSkusBttn
//fastMovingSkusTable.ajax.reload();

$('#getFastMovSkusBttn').click(function () {
  //if (confirm('Do you really want to reset everything to defatul?')) {
  $.ajax({
    url: '../Inventory/GetFastMovingSkus',
    type: "GET",
    contentType: "application/json charset=utf-8",
    dataType: 'json',
    data: { "today": today, "fromClick": "*" },
    success: function () {
      //alert('Successfully send the notification!');
      fastMovingSkusTable.ajax.reload();
      document.getElementById('fstMvSendNotifBttn').disabled = false;
    }
  });
...
// Warehouse #07
$('#warehouseAllFilterCB, #warehouseBancFilterCB, #warehouseBascFilterCB, #warehouseMainSLFilterCB').click(function () {
  invFeedsLeftOverTable.draw(); //, #warehouseSwcaftFilterCB
});
...
$.fn.dataTable.ext.search.push(
  function (settings, data, dataIndex) {
    ...
    var zinusTracyWf = document.getElementById('warehouseZinusTracyFilterCB').checked;
    var zinusChsWf = document.getElementById('warehouseZinusChsFilterCB').checked;
    ...
    if (
      (//(allMf ? (mkf >= 0) : "") ||zinusTracyWf, zinusChsWf
        (noneMf ? (mkf == 0) : "") ||
        (amazonMf ? (mkf == 1) : "") ||
        (bpmMf ? (mkf == 3) : "") ||
        (eBayMf ? (mkf == 5) : "") ||
...
$.fn.dataTable.ext.search.push(
  function (settings, data, dataIndex) {
    ...
    var zinusTracyQty = parseInt($('#zinusTracyThanQty').val(), 10);
    var zinusChsQty = parseInt($('#zinusChsThanQty').val(), 10);
    //var bancInv = parseFloat(data[3]) || 0; // use data for the qty column
    //var bascInv = parseFloat(data[6]) || 0; // use data for the qty column
    var mainslInv = parseFloat(data[3]) || 0; // use data for the qty column
    //var swcaftInv = parseFloat(data[12]) || 0; // use data for the qty column
    //var prismCastInv = parseFloat(data[15]) || 0; // use data for the qty column
    //var prismCaltInv = parseFloat(data[18]) || 0; // use data for the qty column
    var zinusTracyInv = parseFloat(data[6]) || 0; // use data for the qty column
    var zinusChsInv = parseFloat(data[9]) || 0; // use data for the qty column

    //&& isNaN(swcaftQty) && isNaN(prismCastQty) && isNaN(prismCaltQty)
    // || (0 < swcaftInv && swcaftInv <= swcaftQty)
    // || (0 < prismCastInv && prismCastInv <= prismCastQty) || (0 < prismCaltInv && prismCaltInv <= prismCaltQty)
    //isNaN(bancQty) && isNaN(bascQty)  &&
    // || (0 < bancInv && bancInv <= bancQty) || (0 < bascInv && bascInv <= bascQty)
    if ((isNaN(mainSLQty) && isNaN(zinusTracyQty) && isNaN(zinusChsQty))
      || (0 < mainslInv && mainslInv <= mainSLQty)
      || (0 < zinusTracyInv && zinusTracyInv <= zinusTracyQty) || (0 < zinusChsInv && zinusChsInv <= zinusChsQty)) {
      return true;
    }
    return false;
  }
...
$('#fstMvSendNotifBttn').click(function () {
  //if (confirm('Do you really want to reset everything to defatul?')) {
  $.ajax({
    url: '../Inventory/SendFastMovingNotification',
    type: "POST",
    contentType: "application/json charset=utf-8",
    dataType: 'json',
    data: '',
...
window.addEventListener('resize', function (event) {
  // do stuff here
  setTimeout(function () {
    warehousesStatusTable.columns.adjust();
    fastMovingSkusTable.columns.adjust();
    //console.log("Table!");
  }, 500);
});
...
$(document).ready(function () {
  ...
  // Event listener to the two filtering inputs to re-draw on input
  $('#bancLessThanQty, #mainSLThanQty, #bascLessThanQty, #swcaftLessThanQty, #prismCastLessThanQty, #prismCaltLessThanQty, #zinusTracyLessThanQty, #zinusChsLessThanQty').keyup(function () {
  ...
  $('#invMainslBancItems tbody').on('click', 'tr', function () {
    ...
    $("#doNotFeedFromZinusTracy").removeAttr("disabled");
    $("#doNotFeedFromZinusTracyQtyOnHand").removeAttr("disabled");
    $("#doNotFeedFromZinusTracyQtyAvail").removeAttr("disabled");
    $("#doNotFeedFromZinusChs").removeAttr("disabled");
    $("#doNotFeedFromZinusChsQtyOnHand").removeAttr("disabled");
    $("#doNotFeedFromZinusChsQtyAvail").removeAttr("disabled");
    ...
    $("#doNotFeedFromZinusTracy").prop('checked', false);
    document.getElementById('doNotFeedFromZinusTracyQtyOnHand').value = 0;
    document.getElementById('doNotFeedFromZinusTracyQtyAvail').value = 0;
    $("#doNotFeedFromZinusChs").prop('checked', false);
    document.getElementById('doNotFeedFromZinusChsQtyOnHand').value = 0;
    document.getElementById('doNotFeedFromZinusChsQtyAvail').value = 0;
    ...
    document.getElementById('doNotFeedFromStagePOModZinusTracy60').value = 0;
    document.getElementById('doNotFeedFromStagePOModZinusChs60').value = 0;
    ...
    document.getElementById('doNotFeedFromStagePOModZinusTracy90').value = 0;
    document.getElementById('doNotFeedFromStagePOModZinusChs90').value = 0;
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $('#backOrderLeadTimeZinusTracy').prop('selectedIndex', (data.checkBackOrderLeadTimeZinusTracy - 2));
        $('#backOrderLeadTimeZinusChs').prop('selectedIndex', (data.checkBackOrderLeadTimeZinusChs - 2));
        ...
        $("#doNotFeedFromZinusTracy").prop('checked', data.doNotFeedFromZinusTracy);
        document.getElementById('doNotFeedFromZinusTracyQtyOnHand').value = data.qtyOnHandZinusTracy;
        document.getElementById('doNotFeedFromZinusTracyQtyAvail').value = data.qtyAvailZinusTracy;
        $("#doNotFeedFromZinusChs").prop('checked', data.doNotFeedFromZinusChs);
        document.getElementById('doNotFeedFromZinusChsQtyOnHand').value = data.qtyOnHandZinusChs;
        document.getElementById('doNotFeedFromZinusChsQtyAvail').value = data.qtyAvailZinusChs;
        ...
        document.getElementById('doNotFeedFromStagePOModZinusTracy60').value = data.stagePOModZinusTracy60;
        document.getElementById('doNotFeedFromStagePOModZinusChs60').value = data.stagePOModZinusChs60;
        ...
        document.getElementById('doNotFeedFromStagePOModZinusTracy90').value = data.stagePOModZinusTracy90;
        document.getElementById('doNotFeedFromStagePOModZinusChs90').value = data.stagePOModZinusChs90;
        ...
        */
        if (data.doNotFeedFromZinusTracy == true) {
          $("#doNotFeedFromZinusTracyQtyOnHand").attr("disabled", "disabled");
          $("#doNotFeedFromZinusTracyQtyAvail").attr("disabled", "disabled");
          $("#doNotFeedFromStagePOModZinusTracy60").attr("disabled", "disabled");
          $("#doNotFeedFromStagePOModZinusTracy90").attr("disabled", "disabled");
        }

        if (data.doNotFeedFromZinusChs == true) {
          $("#doNotFeedFromZinusChsQtyOnHand").attr("disabled", "disabled");
          $("#doNotFeedFromZinusChsQtyAvail").attr("disabled", "disabled");
          $("#doNotFeedFromStagePOModZinusChs60").attr("disabled", "disabled");
          $("#doNotFeedFromStagePOModZinusChs90").attr("disabled", "disabled");
        }
        // alert("Yay!");
      }
    });
  ...
  $("#doNotFeedFromAll").click(function () {
    if ($(this).is(":checked")) {
      ...
      */
      ...
      document.getElementById('doNotFeedFromZinusTracy').checked = true;
      $("#doNotFeedFromZinusTracy").attr("disabled", "disabled");
      $("#doNotFeedFromZinusTracyQtyOnHand").attr("disabled", "disabled");
      $("#doNotFeedFromZinusTracyQtyAvail").attr("disabled", "disabled");

      document.getElementById('doNotFeedFromZinusChs').checked = true;
      $("#doNotFeedFromZinusChs").attr("disabled", "disabled");
      $("#doNotFeedFromZinusChsQtyOnHand").attr("disabled", "disabled");
      $("#doNotFeedFromZinusChsQtyAvail").attr("disabled", "disabled");
      ...
      */
      ...
      $("#doNotFeedFromPrismCaltQtyAvail").removeAttr("disabled");
      */
      document.getElementById('doNotFeedFromZinusTracy').checked = false;
      $("#doNotFeedFromZinusTracy").removeAttr("disabled");
      $("#doNotFeedFromZinusTracyQtyOnHand").removeAttr("disabled");
      $("#doNotFeedFromZinusTracyQtyAvail").removeAttr("disabled");

      document.getElementById('doNotFeedFromZinusChs').checked = false;
      $("#doNotFeedFromZinusChs").removeAttr("disabled");
      $("#doNotFeedFromZinusChsQtyOnHand").removeAttr("disabled");
      $("#doNotFeedFromZinusChsQtyAvail").removeAttr("disabled");
      ...
      $("#doNotFeedFromStagePOModZinusTracy60").removeAttr("disabled");
      $("#doNotFeedFromStagePOModZinusChs60").removeAttr("disabled");
      ...
      $("#doNotFeedFromStagePOModZinusTracy90").removeAttr("disabled");
      $("#doNotFeedFromStagePOModZinusChs90").removeAttr("disabled");
...
$(function () {
  $("#doNotFeedFromMainsl").click(function () {
    if ($(this).is(":checked") && (document.getElementById('doNotFeedFromBanc').checked == false)) {
      ...
      && (document.getElementById('doNotFeedFromZinusTracy').checked == true) && (document.getElementById('doNotFeedFromZinusChs').checked == true)) {
  ...
  */
  ...
  $("#doNotFeedFromPrismCast").click(function () {
    if ($(this).is(":checked") && (document.getElementById('doNotFeedFromMainsl').checked == false)) {
      ...
      && (document.getElementById('doNotFeedFromBanc').checked == true) //&& (document.getElementById('doNotFeedFromSwcaft').checked == true) 
      && (document.getElementById('doNotFeedFromBasc').checked == true) && (document.getElementById('doNotFeedFromPrismCalt').checked == true)
      && (document.getElementById('doNotFeedFromZinusTracy').checked == true) && (document.getElementById('doNotFeedFromZinusChs').checked == true)) {
      $("#doNotFeedFromAll").click();
    } else {
      $("#doNotFeedFromPrismCastQtyOnHand").removeAttr("disabled");
  ...
  $("#doNotFeedFromPrismCalt").click(function () {
    if ($(this).is(":checked") && (document.getElementById('doNotFeedFromMainsl').checked == false)) {
      ...
      && (document.getElementById('doNotFeedFromBanc').checked == true) //&& (document.getElementById('doNotFeedFromSwcaft').checked == true) 
      && (document.getElementById('doNotFeedFromPrismCast').checked == true) && (document.getElementById('doNotFeedFromBasc').checked == true)
      && (document.getElementById('doNotFeedFromZinusTracy').checked == true) && (document.getElementById('doNotFeedFromZinusChs').checked == true)) {
      $("#doNotFeedFromAll").click();
    } else {
      $("#doNotFeedFromPrismCaltQtyOnHand").removeAttr("disabled");
  ...
  */
  $("#doNotFeedFromZinusTracy").click(function () {
    if ($(this).is(":checked") && (document.getElementById('doNotFeedFromMainsl').checked == false)) {
      $("#doNotFeedFromZinusTracyQtyOnHand").attr("disabled", "disabled");
      $("#doNotFeedFromZinusTracyQtyAvail").attr("disabled", "disabled");
      $("#doNotFeedFromStagePOModZinusTracy60").attr("disabled", "disabled");
      ...
      && (document.getElementById('doNotFeedFromBanc').checked == true) //&& (document.getElementById('doNotFeedFromSwcaft').checked == true) 
      && (document.getElementById('doNotFeedFromPrismCast').checked == true) && (document.getElementById('doNotFeedFromPrismCalt').checked == true)
      && (document.getElementById('doNotFeedFromBasc').checked == true) && (document.getElementById('doNotFeedFromZinusChs').checked == true)) {
      $("#doNotFeedFromAll").click();
    } else {
      $("#doNotFeedFromZinusTracyQtyOnHand").removeAttr("disabled");
      $("#doNotFeedFromZinusTracyQtyAvail").removeAttr("disabled");
      $("#doNotFeedFromStagePOModZinusTracy60").removeAttr("disabled");
      $("#doNotFeedFromStagePOModZinusTracy90").removeAttr("disabled");
      $("#doNotFeedFromZinusTracy").focus();
    }
  });

  $("#doNotFeedFromZinusChs").click(function () {
    if ($(this).is(":checked") && (document.getElementById('doNotFeedFromMainsl').checked == false)) {
      $("#doNotFeedFromZinusChsQtyOnHand").attr("disabled", "disabled");
      $("#doNotFeedFromZinusChsQtyAvail").attr("disabled", "disabled");
      $("#doNotFeedFromStagePOModZinusChs60").attr("disabled", "disabled");
      ...
      && (document.getElementById('doNotFeedFromBanc').checked == true) //&& (document.getElementById('doNotFeedFromSwcaft').checked == true) 
      && (document.getElementById('doNotFeedFromPrismCast').checked == true) && (document.getElementById('doNotFeedFromPrismCalt').checked == true)
      && (document.getElementById('doNotFeedFromZinusTracy').checked == true) && (document.getElementById('doNotFeedFromBasc').checked == true)) {
      $("#doNotFeedFromAll").click();
    } else {
      $("#doNotFeedFromZinusChsQtyOnHand").removeAttr("disabled");
      $("#doNotFeedFromZinusChsQtyAvail").removeAttr("disabled");
...
$("#updateInvFeedsSKUDetail").click(function () {
  ...
  */
  ...
  returnResult.DoNotFeedFromZinusTracy = $('#doNotFeedFromZinusTracy').is(':checked');
  returnResult.QtyOnHandZinusTracy = $('#doNotFeedFromZinusTracyQtyOnHand').val();
  returnResult.QtyAvailZinusTracy = $('#doNotFeedFromZinusTracyQtyAvail').val();
  returnResult.DoNotFeedFromZinusChs = $('#doNotFeedFromZinusChs').is(':checked');
  returnResult.QtyOnHandZinusChs = $('#doNotFeedFromZinusChsQtyOnHand').val();
  returnResult.QtyAvailZinusChs = $('#doNotFeedFromZinusChsQtyAvail').val();
  ...
  returnResult.StagePOModZinusTracy60 = $("#doNotFeedFromStagePOModZinusTracy60").val();
  returnResult.StagePOModZinusChs60 = $("#doNotFeedFromStagePOModZinusChs60").val();
  ...
  returnResult.StagePOModZinusTracy90 = $("#doNotFeedFromStagePOModZinusTracy90").val();
  returnResult.StagePOModZinusChs90 = $("#doNotFeedFromStagePOModZinusChs90").val();
  ...
  returnResult.checkBackOrderLeadTimeZinusTracy = document.getElementById("backOrderLeadTimeZinusTracy").value;
  returnResult.checkBackOrderLeadTimeZinusChs = document.getElementById("backOrderLeadTimeZinusChs").value;
...
function importFastMovingSkus() {
  ...
  if (hasUpd == 1) {
    ...
    document.getElementById('fstMvSendNotifBttn').disabled = true;
    document.getElementById('getFastMovSkusBttn').disabled = true;
...
var warehousesStatusTable = $("#invMainslBancItems").DataTable({
  ...
  "columns": [
    ...
    { "data": "qtyAvailMainsl", "width": "9%" },
    { "data": "stagePOOrigMainsl60", "width": "9%" },
    { "data": "stagePOOrigMainsl90", "width": "9%" },
    ...
    { "data": "qtyAvailZinusTracy", "width": "9%" },
    { "data": "stagePOOrigZinusTracy60", "width": "9%" },
    { "data": "stagePOOrigZinusTracy90", "width": "9%" },
    { "data": "qtyAvailZinusChs", "width": "9%" },
    { "data": "stagePOOrigZinusChs60", "width": "9%" },
    { "data": "stagePOOrigZinusChs90", "width": "9%" }
  ],
  "language": {
...
var fastMovingSkusTable = $("#invFastMovingSkus").DataTable({
  "destroy": true,
  "scrollX": true,
  "scrollCollapse": true,
  //"paging": false,
  ...
  "order": [[6, "dsc"]],
  dom: 'Bfrtip',
  lengthMenu: [[20, 30, 50, 100], [15, 25, 50, 100]],
  buttons: [
    {
      extend: 'copyHtml5',
      filename: function () {
        ...
        var todayDate = (tmpToday.getMonth() + 1).toString().padStart(2, '0') + '/' + tmpToday.getDate().toString().padStart(2, '0') + '/' + tmpToday.getFullYear();
        var todatyTime = tmpToday.getHours().toString().padStart(2, '0') + tmpToday.getMinutes().toString().padStart(2, '0') + tmpToday.getSeconds().toString().padStart(2, '0');
        return "Fast_Moving_SKUs" + "_" + todayDate + "_" + todatyTime;
      },
      exportOptions: {
        columns: ':visible'
      }
  ...
  "columns": [
    { "data": "itemNoId", "visible": false },
    { "data": "itemName", "width": "17%" },
    { "data": "qtyAvailMainsl", "width": "17%" },
    //{ "data": "qtyAvailBanc", "width": "11%" },
    //{ "data": "qtyAvailBasc", "width": "11%" },
    { "data": "qtyAvailZinusTracy", "width": "17%" },
    { "data": "qtyAvailZinusChs", "width": "17%" },
    { "data": "totalQty", "width": "17%" },
    { "data": "totalQtySales", "width": "17%" }
  ],
  "language": {
    "emptyTable": "no data found"
  },
